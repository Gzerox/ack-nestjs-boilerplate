...
[ACK NestJs][ref-ack] is a [NestJs v12.x][ref-nestjs] boilerplate with JWT, OAuth (Google & Apple), TOTP/2FA, and scoped roles with CASL policies. It runs Prisma on **PostgreSQL**, uses native ESM, and stays modular with a repository layer for data access.
...
- PostgreSQL 18 is required: every id is a UUID v7 generated by the database through `uuidv7()`.
...
3. **[Prisma ORM][ref-prisma]** - Schema design, migrations, and type-safe queries
4. **[PostgreSQL][ref-postgresql]** - Relational basics, transactions, and UUID keys
...
- **Prisma on PostgreSQL** - Type-safe queries, native transactions, and versioned migrations
...
# Apply the schema migrations (PostgreSQL must already be up)
pnpm db:migrate
...
- Schema changes: `pnpm db:migrate` (`prisma migrate dev`), which writes and applies a migration file under `prisma/migrations/`
- Ids: UUID v7, generated by the database
- Transactions are native; no replica set is involved

Setup and seeding: [Database Documentation][ref-doc-database].

## Installation

Docker is the recommended setup. Step-by-step (Compose first, then hosted PostgreSQL + Redis if you cannot use Docker): [Installation][ref-doc-installation].
